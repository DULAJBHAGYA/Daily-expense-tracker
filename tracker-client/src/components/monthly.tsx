...
const MonthlySummary: React.FC = () => {
  ...
  const fetchMonthlySummary = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/expenses/summary/march-2026");
  ...
  return (
    <div className="p-4 sm:p-6">
      <h2 className="text-xl font-semibold mb-4 text-gray-900">
        Monthly Summary from March 2026
      </h2>
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {summary.map((item) => (
                  <tr key={`${item.month}-${item.year}`} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">
                      {`${item.month} ${item.year}`}
                    </td>
                    <td className="px-6 py-4 text-[#15994e] font-semibold">
                      {item.income.toFixed(2)}
                    </td>
                    ...
                    <td
                      className={`px-6 py-4 font-bold ${
                        item.income - item.expense >= 0 ? "text-[#15994e]" : "text-red-700"
                      }`}
                    >
                      {(item.income - item.expense).toFixed(2)}
                    </td>
